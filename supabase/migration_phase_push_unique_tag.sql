-- migration_phase_push_unique_tag.sql
-- Make each push notification get a unique tag so the browser stacks them
-- in the notification drawer instead of replacing prior unread ones.
-- (Old behaviour: all 'mention' pushes shared tag='mention' → 2nd replaced 1st.)
--
-- Tag format: '<kind>:<notification_id>' — kind kept as a prefix so debugging
-- by category is still easy.
...
begin
  ...
  perform net.http_post(
    ...
    body    := jsonb_build_object(
      ...
      'url',     nav_url,
      'tag',     new.kind || ':' || new.id::text   -- unique per notification
    )
  );
